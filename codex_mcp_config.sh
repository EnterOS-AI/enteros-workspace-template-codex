...
#
# The surgical fix (verified working in core#2128) is to select the
# `workspace-write` sandbox policy AND set `network_access = true` under
# `[sandbox_workspace_write]`. With network access explicitly granted,
# codex does NOT unshare the net namespace, so bwrap never attempts the
# RTM_NEWADDR that the kernel rejects. This keeps codex's *filesystem*
# sandbox intact (agents execute arbitrary model-generated code, so a
# write-scoped sandbox is worth keeping) while restoring outbound
# network — strictly more secure than fully neutering bwrap. An earlier
# WIP (branch fix/bwrap-shim-net_admin-blocker) instead replaced the
# bundled bwrap with a passthrough shim; that drops the fs sandbox
# entirely, so we prefer this config-only fix. See the PR body.
#
...
# Strip any previous auto-generated sandbox stanza: the top-level
# sandbox_mode line, its marker comment, and the whole
# [sandbox_workspace_write] table (header through next [section] / EOF).
...
# Prepend the top-level sandbox_mode key (before any [table] header).
{
...
# Append the [sandbox_workspace_write] table granting network access.
cat >> "$CONFIG_TOML" <<'EOF'

# Auto-generated by codex_mcp_config.sh — sandbox network fix (core#2128).
# network_access=true stops codex from unsharing the net namespace, which
# is what made bwrap fail RTM_NEWADDR on the workspace AMI kernel.
...
echo "[codex-mcp] wrote ${CONFIG_TOML} mcp_servers.molecule python=${PYTHON_BIN} workspace_id=${WORKSPACE_ID_VAL:-<unset>} sandbox=workspace-write network_access=true"
